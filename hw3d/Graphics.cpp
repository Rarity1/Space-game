...
Graphics::Graphics(HWND* hWnd, int height, int width, Keyboard* kbd)
	:
	width(width),
	height(height),
	hWnd(hWnd),
...
void Graphics::LoadResources()
{
	...
	lModels = std::make_unique<RStorage>(commandList, pDevice, commandAllocator, commandQueue);
	...
	//Model Loading
	modelVect.emplace_back(lModels->Read("cube"));
	modelVect.emplace_back(lModels->Read("untitled"));

	//End model loading
	lModels->CreateBuffers(modelVect);
	//Deferr SRV descriptor to get model count
	{
		D3D12_DESCRIPTOR_HEAP_DESC dsc = {};
		...
		NAME_D3D12_OBJECT(srvDescriptorHeap);
	}
...
//Better to set model position manually than use this
void Graphics::SetModelPosition(std::string name, XMFLOAT3 position, RStorage::aRotation rotation, RStorage::pChange which) {
	for (auto& m : modelVect) {
		if (m->name == name) {
			SetModelPosition(m, position, rotation, which);
		}
	}

}
void Graphics::SetModelPosition(RStorage::bmResource* model, XMFLOAT3 position, RStorage::aRotation rotation, RStorage::pChange which) {
	...
	case RStorage::BOTH: {
		...
		model->changepos = RStorage::BOTH;
		break;
	}case RStorage::ROTATION: {
		model->rotation = rotation;
		model->changepos = RStorage::ROTATION;
		break;
	}case RStorage::POSITION: {
		model->position = position;
		model->changepos = RStorage::POSITION;
		break;
	}
	}
}
...
void Graphics::SetModelVectPositions(std::vector<rpVect> rpVects) {
	for (auto& m : rpVects) {
		SetModelPosition(m.model, m.position, m.rotation, m.which);
	}
}
...
void Graphics::OnUpdate() {
	...
	while (auto ss = kbd->ReadKey()) {
		if (ss->IsPress()) {
			camera.OnKeyDown(ss->GetCode());
		}
		else {
			camera.OnKeyUp(ss->GetCode());
		}
	}
	auto temp = 0;
	timesincestart -= timer.Peek();
	RStorage::aRotation ttime = { 0, 0, 0 };

	for (auto& m : modelVect) {
		SetModelPosition(m, {0,(float)temp,0}, ttime, RStorage::BOTH);
		temp += 10;
	}
	
	camera.Update(timer.Peek());
	timer.Mark();
}
...
void Graphics::CreateFrameResources() {
	...
	for (UINT i = 0; i < bufferCount; i++)
	{
		...
		// Describe and create a constant buffer view (CBV).
			...
			pDevice->CreateShaderResourceView(m->tbuffer.Get(), &srvDesc, cbvSrvHandle);
			cbvSrvHandle.Offset(srvDescriptorSize);
