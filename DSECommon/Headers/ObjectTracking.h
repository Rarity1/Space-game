...
public:
  ...
  class Instance {
    ...
    void LinkToModel(Object* obj, umID umID);

    bool operator==(Instance& comp){
      return comp.instanceID == instanceID;
    }

    public:
    Instance(InstID instanceID, Tracker &Parent);
    Tracker &pTracker;
    const std::unordered_map<UINT, std::list<Object *>>& GetRenderObjects(){
      return ModelLinkedObjects;
    }
    const std::list<Object*>& GetPhysicsObjects(){
      return PhysicsObjects;
    }
    CBVData*& GetCBVPtr(umID ID){
      return instancedCBVData[ID];
    }
  ...
  Tracker(RStorage &rs) : storage(rs) {};
  ~Tracker() {};
  ...
  Instance &initInstance(InstID instanceID = 0);
...
public:
  ...
  relposVect(DirectX::XMFLOAT3 initPos, DirectX::XMFLOAT4 initRot): position(std::make_unique<DirectX::XMFLOAT3>(initPos)),
  rotation(initRot)
  {};
  ~relposVect() = default;
...
protected:
  std::string name = "";
  UOID uOID;
  std::function<void()> ActiveFunction;
  Object* Parent;
  std::vector<Object*> Children;
  Tracker::Instance* LinkedInstance;
public:
  Object(std::string name, UOID uOID, 
    std::function<void()> func = []{}, 
...
public:
  ...
  Object(obj),
  loadedModel(obj.loadedModel),
  isHidden(obj.isHidden.load()), 
  model(obj.model),
  CBVIndex(obj.CBVIndex),
  scale(obj.scale)
  {};
};
...
class PhysicsObject : public RenderedObject{
  ...
  PhysicsObject(RenderedObject& obj, float mMass = 1, float mFriction = 0, 
    DirectX::XMFLOAT3 initVelDir = {0,0,0}, 
    float initSpeed = 0);
  PhysicsObject(std::string name, UOID uOID,
    std::function<void()> func = []{}, 
    Tracker::Instance* pInstance = nullptr,
    RStorage::bmResource* model = nullptr, 
    umID filebModelIndex = 0, float mScale = 1, 
    DirectX::XMFLOAT3 initPos = {0,0,0}, 
    DirectX::XMFLOAT4 initRot = {0,0,0,1},
    float mMass = 1, float mFriction = 0, 
    DirectX::XMFLOAT3 initVelDir = {0,0,0}, 
    float initSpeed = 0
  );
