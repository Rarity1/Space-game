...
#include <sstream>
  ...
  static void GetAllChildBones(Node &node, std::vector<Node *> &Parent) {
...
static Node ChildNodeRead(rapidxml::xml_node<char>& node) {
  Node mParent;
  {
  ...
  if (std::strcmp(node.first_attribute("type")->value(), "JOINT") == 0 &&
      node.first_node("extra")->first_node("technique")->first_node("tip_x") !=
          nullptr) {
    float x;
    std::stringstream(node.first_node("extra")
                          ->first_node("technique")
                          ->first_node("tip_x")
                          ->value()) >>
        x;
    float y;
    ...
    std::stringstream(node.first_node("extra")
        ...
        z;
    //float roll;
    //std::stringstream(node.first_node("extra")->first_node("technique")->first_node("roll")->value())
    //>> roll;
    mParent.pos = {x, y, z, 0.f};
  }
  
  for (auto tempnode = node.first_node("node"); tempnode;
       tempnode = tempnode->next_sibling()) {
    if (tempnode != nullptr && tempnode->first_attribute("type") != nullptr) {
      if (std::strcmp(tempnode->first_attribute("type")->value(), "JOINT") ==
          0) {
...
ModelData::ModelData(std::string path) : Path(path) {
  rapidxml::xml_document<char> doc;
  std::vector<char> buffer;
  {
    auto file = std::ifstream(Path);

    buffer.append_range(std::vector<char>(std::istreambuf_iterator<char>(file),
  ...
  for (rapidxml::xml_node<> *node = tnode->first_node(); node;
       node = node->next_sibling()) {
    if (std::strcmp(node->name(), "library_geometries") == 0) {
      auto source =
          node->first_node("geometry")->first_node("mesh")->first_node();
      while (source != nullptr) {
        if (source->first_attribute() != nullptr)
          if ((std::strstr((source->first_attribute()->value()), "positions") !=
               nullptr) &&
              (positions == nullptr)) {
            positions = source->first_node("float_array");
            auto ssource =
                source->first_node("technique_common")->first_node("accessor");
            std::stringstream(ssource->first_attribute("count")->value()) >>
                size;
          } else if ((std::strstr((source->first_attribute()->value()),
                                  "normals") != nullptr) &&
                     (pnorm == nullptr)) {
    ...
    if (std::strcmp(node->name(), "library_controllers") == 0) {
      auto source =
          node->first_node("controller")->first_node("skin")->first_node();
      while (source != nullptr) {
        if (source->first_attribute() != nullptr)
          if ((std::strstr(source->first_attribute()->value(), "joints") !=
               nullptr) &&
              (bonenames == nullptr)) {
  ...
  // Idata only correct if inverted with RH view because collada is counter clockwise
  ...
  auto count = 1;
  for(auto& bon : bdata){
    for(auto& in : bon.Indices){
      auto& vertex = MappedVertices[IndexToVertex[in]];
      if(vertex[0].test== 0){
      //vertex[0].test = count;
      //vertex[1].test = count;
      //vertex[2].test = count;
  ...
  for (auto &b : bdata) {
    FLOAT3 pos{0, 0, 0};
    std::for_each(b.Indices.begin(), b.Indices.end(),
                  [&pos, &mVdata](auto &x) { pos = mVdata[x].position + pos; });
